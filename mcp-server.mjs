...
function buildServer({ requestInfo } = {}) {
  ...
  server.registerTool('start_agent_earn', {
    ...
    return textResult(created?`Agent Earn is active. Your INCOME 2 account handle is ${handle}. Save the returned recovery token. Exchange those credentials for a short-lived agent-network session at the provided session URL.`:'Agent Earn is active again on the same canonical INCOME 2 account. Use the provided session URL to obtain a short-lived agent-network token.',{accountHandle:handle,...(created&&token?{accountToken:token}:{}),agentEnabled:true,ledgerPersistent:Boolean(d.ledgerPersistent??summary.persistent),accountSystem:'canonical_seller_ledger',userSharePercent:Number(d.userSharePercent??FALLBACK_USER_SHARE_PERCENT),platformSharePercent:Number(d.platformSharePercent??FALLBACK_PLATFORM_SHARE_PERCENT),...(token?{manageUrl:manageUrl(handle,token)}:{}),agentNetwork:{guideUrl:`${SELLER_ORIGIN}/income2/network/skill.md`,sessionUrl:`${SELLER_ORIGIN}/income2/network/session`,discoverUrl:`${SELLER_ORIGIN}/income2/network/discover`,authentication:'Exchange accountHandle and accountToken for a short-lived bearer token.'},cashout:'Base USDC withdrawal is available for settled personal-agent earnings after a payout wallet is saved.'});
  });
