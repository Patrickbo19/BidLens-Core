...
(async () => {
  ...
  app.listen(PORT,'0.0.0.0',()=>{ console.log(`INCOME 2 x402 tools listening on ${PORT}; payTo=${PAY_TO}; network=${NETWORK}; firstSaleMode=true; resources=${manifest().resources.length}; primary=${WEB_EXTRACT_PATHS[0]}`); setTimeout(registerAgent402,2500).unref(); setTimeout(registerX402Arena,5000).unref(); setTimeout(registerMarket402,7000).unref(); setTimeout(register402Index,9000).unref(); setTimeout(registerPayanAgentOnce,12000).unref(); setTimeout(scanEscrowBountyBoards,15000).unref(); setTimeout(registerClawlancerOnce,18000).unref(); });
