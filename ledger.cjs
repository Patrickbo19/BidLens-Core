...
function newHandle() {
  return `earn_${crypto.randomBytes(8).toString('hex')}`;
}
...
async function createAccount({ enableAgent = true } = {}) {
  ...
  if (!pool) {
    memory.accounts.set(handle, {
      id,
      handle,
      tokenHash,
      agentEnabled: Boolean(enableAgent),
      createdAt: now,
      updatedAt: now,
      lastCreditAt: null,
    });
    ...
    await pool.query(
      `INSERT INTO earn_accounts (id, handle, token_hash, agent_enabled)
       VALUES ($1, $2, $3, $4)`,
...
async function setAgentEnabled(handle, token, enabled) {
  ...
  if (!pool) {
    ...
    await pool.query(
      `UPDATE earn_accounts SET agent_enabled = $1, updated_at = now() WHERE id = $2`,
      [Boolean(enabled), account.id],
    );
  }
...
async function getSummary(handle, token) {
  ...
  const result = await pool.query(
    `SELECT id, source, source_ref, status, gross_usd, user_share_usd,
            platform_share_usd, payer, network, asset, metadata, created_at
       FROM earn_ledger_entries
      WHERE account_id = $1
...
function buildSummary(account, entries) {
  ...
  const gross = money(settled.reduce((n, e) => n + Number(e.grossUsd || 0), 0));
  const userShare = money(settled.reduce((n, e) => n + Number(e.userShareUsd || 0), 0));
  const platformShare = money(settled.reduce((n, e) => n + Number(e.platformShareUsd || 0), 0));
  return {
    handle: account.handle,
    agentEnabled: Boolean(account.agentEnabled),
    persistent: Boolean(pool),
    grossAttributedUsd: gross,
    availableBalanceUsd: userShare,
...
async function recordAgentSettlement({ sourceRef, grossUsd, payer, transaction, network = 'eip155:8453', asset = 'USDC', metadata = {} }) {
  ...
  if (!pool) {
    ...
    const active = [...memory.accounts.values()]
      .filter(a => a.agentEnabled)
      .sort((a, b) => String(a.lastCreditAt || '').localeCompare(String(b.lastCreditAt || '')) || a.createdAt.localeCompare(b.createdAt))[0] || null;
  ...
  try {
    ...
    await client.query(
      `INSERT INTO earn_ledger_entries
        (id, account_id, source, source_ref, status, gross_usd, user_share_usd,
         platform_share_usd, payer, network, asset, metadata)
    ...
    if (active) {
      await client.query(`UPDATE earn_accounts SET last_credit_at = now(), updated_at = now() WHERE id = $1`, [active.id]);
    }
...
async function systemStatus() {
  ...
  const [a, e] = await Promise.all([
    pool.query(`SELECT count(*)::int AS accounts, count(*) FILTER (WHERE agent_enabled)::int AS active FROM earn_accounts`),
