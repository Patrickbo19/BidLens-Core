...
> **Takeover reconciliation (2026-09-13):** Read `INCOME2_AUDIT_2026-09-13.md` before operating. It supersedes stale live-status claims below. CDP credentials are configured; router A2A/ARD/Agent Magnet routes are verified live; and all five personal routes now return valid x402/Bazaar payment challenges to the separate treasury. Agent402 still labels those five routes `paid:false` after an explicit recrawl, so do not claim registry paid classification. Patrick confirmed on September 13 that he does not own the payer wallet for the September 12 $0.001 on-chain settlement. That settlement is therefore the first verified unrelated outside buyer; production logs also verify successful fulfillment. Verified unrelated-buyer revenue is **$0.001**. The older $0.003 settlement and owner funding remain excluded.
