...
function fixedRef(req,tool){const proof=String(req.headers['payment-signature']||req.headers['x-payment']||req.headers['payment']||'');return crypto.createHash('sha256').update(`income2|${tool}|${proof}|${req.ip||''}`).digest('hex')}
...
async function reserveWorker(){const list=await workers();if(!list.length)return null;const w=list[0];await pool.query(`UPDATE income2_personal_agents SET last_scan_at=now(),opportunity_count=opportunity_count+1,updated_at=now() WHERE account_id=$1`,[w.account_id]);return w}
...
function install(){
  ...
  express.application.listen=function income2MarketListen(...args){
    if(!this.__income2MarketRoutes){
      ...
      this.get('/income2-market/status',async(_req,res)=>{const list=await workers().catch(()=>[]);res.json({ok:true,service:'income2-personal-agent-market',paidTools:Object.keys(TOOLS),payoutReadyWorkers:list.length,network:NETWORK,asset:'USDC',priceUsd:PRICE_USD,privateEarnExcluded:true})});
      for(const [name,tool] of Object.entries(TOOLS)){
        this.post(`/income2-market/${name}`,async(req,res,next)=>{
          try{
            if(!/^0x[a-fA-F0-9]{40}$/.test(PAY_TO))return res.status(503).json({ok:false,message:'personal-agent market payment receiver unavailable'});
            const worker=await reserveWorker();if(!worker)return res.status(503).json({ok:false,message:'No payout-ready personal agents yet.'});
            req.__income2Worker=worker;
            const {paymentMiddleware,rs}=await runtime();
            const mw=paymentMiddleware({[`POST ${req.path}`]:{accepts:[{scheme:'exact',price:PRICE,network:NETWORK,payTo:PAY_TO}],description:tool.description,mimeType:'application/json'}},rs);
            return mw(req,res,async err=>{if(err)return next(err);try{const result=tool.run(req.body||{});const recorded=await personal.recordEarning({id:worker.account_id},{source:`income2_market:${name}`,sourceRef:fixedRef(req,name),grossUsd:PRICE_USD,metadata:{agentId:worker.agent_id,route:req.path}});return res.json({ok:true,result,workerAgentId:worker.agent_id,personalEarningRecorded:recorded.recorded,priceUsd:PRICE_USD})}catch(e){return next(e)}});
          }catch(e){return next(e)}
        });
      }
      personal.init().then(()=>console.log(JSON.stringify({type:'income2_personal_market_ready',tools:Object.keys(TOOLS).length,priceUsd:PRICE_USD,privateEarnExcluded:true,at:new Date().toISOString()}))).catch(e=>console.error(JSON.stringify({type:'income2_personal_market_init_error',error:String(e.message).slice(0,300),at:new Date().toISOString()})));
    }
