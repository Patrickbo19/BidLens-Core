...
function install(){
  ...
  express.application.listen=function income2MarketListen(...args){
    if(!this.__income2MarketRoutes){
      ...
      this.get('/income2-market/status',async(_req,res)=>{const list=await workers().catch(()=>[]);res.json({ok:true,service:'income2-personal-agent-market',paidTools:Object.keys(TOOLS),payoutReadyWorkers:list.length,bootstrapActive:true,network:NETWORK,asset:'USDC',priceUsd:PRICE_USD,privateEarnExcluded:true})});
        ...
        this.post(`/income2-market/${name}`,async(req,res,next)=>{
          try{
            ...
            return mw(req,res,async err=>{if(err)return next(err);try{
              ...
              if(worker){
                recorded=await personal.recordEarning({id:worker.account_id},{source:`income2_market:${name}`,sourceRef:settlementRef(req,name),grossUsd:PRICE_USD,metadata:{agentId:worker.agent_id,route:req.path}});
      ...
      personal.init().then(()=>console.log(JSON.stringify({type:'income2_personal_market_ready',tools:Object.keys(TOOLS).length,priceUsd:PRICE_USD,bootstrapActive:true,privateEarnExcluded:true,at:new Date().toISOString()}))).catch(e=>console.error(JSON.stringify({type:'income2_personal_market_init_error',error:String(e.message).slice(0,300),at:new Date().toISOString()})));
    }
