...
const PRICES = { sellerStatus: '$0.001', jsonQa: '$0.001', promptScan: '$0.001', urlAudit: '$0.001' };
...
function manifest() {
  return {
    ...
    homepage: 'https://earn-router.onrender.com', openapi: `${ORIGIN}/openapi.json`,
...
function openApi() {
  return { openapi: '3.1.0', info: { title: 'Earn Agent Tools', version: '0.6.1', description: 'Ultra-low-cost deterministic pay-per-call tools for AI agents over x402.' }, servers: [{ url: ORIGIN }], paths: {
...
async function registerAgent402() {
  try {
    ...
    console.error(JSON.stringify({ type: 'agent402_registration_error', origin: ORIGIN, error: String(error?.message || error).slice(0, 500) }));
  }
}
...
(async () => {
  ...
  app.get('/health', async (_req, res) => res.json({ ok: true, service: 'earn-tools-backend', version: '0.6.1', x402: true, network: NETWORK, facilitator: 'payai', firstSaleMode: true, prices: PRICES, ledger: await ledger.systemStatus().catch(() => ({ persistent: false })) }));
